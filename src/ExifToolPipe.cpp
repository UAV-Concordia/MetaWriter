...
ExifToolPipe::ExifToolPipe()
            : mFile(-1), mBuff(NULL), mSize(0), mLen(0), mPos(0), mSearchPos(0),
              mBlockSize(0), mString(NULL), mPid(-1)
{
}
...
int ExifToolPipe::Read()
{
    ...
    for (;;) {
        ...
        int bytesRead = read(mFile, mBuff + mLen, remaining);
        if (bytesRead < 0) {
        ...
        // must null terminate response string for searching
        mBuff[mLen] = '\0';
        //                           0123456789012
        // response should end with "{ready12345}\n" or "{ready12345}\r\n"
