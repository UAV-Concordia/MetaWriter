...
ExifTool::ExifTool(const char *exec, const char *arg1)
        : mWriteInfo(NULL), mCmdQueue(NULL), mCmdQueueLen(0), mCmdQueueSize(0),
          mWatchdog(-1), mLastComplete(-1)
{
    int to[2], from[2], err[2];
    static const char *args[] = {
        NULL, kDefaultExec, "-stay_open", "true", "-@", "-", NULL
    };
    int firstArg = 1;
    if (arg1) {
        args[1] = arg1;
...
int ExifTool::SetNewValue(const char *tag, const char *value, int len)
{
    ...
    if (tag) {
        ...
        if (!info->name) { delete info; return -1; }
        ...
        if (value) {
            ...
            if (len) {
                ...
                if (!info->value) { delete info; return -1; }
...
int ExifTool::WriteInfo(const char *file, const char *opts, TagInfo *info)
{
    ...
    int olen = (opts ? strlen(opts)+1 : 0) + 12; 
    int size = kBlockSize;
    strcpy(buff,file);
    int pos = strlen(file);
    buff[pos++] = '\n';
...
int ExifTool::Command(const char *cmd)
{
    ...
    if (mCmdQueue) {
        n = write(mTo, mCmdQueue, mCmdQueueLen);
        if (n < 0) {
    ...
    if (cmd) {
        ...
        return -4;  // cmd is NULL, and queue is not empty
    }
    return cmdNum;
}
...
int ExifTool::Complete(double timeout)
{
    ...
    if (cmdNum > 0) {
        ...
        for (;;) {
            int n = mStderr.Read();
            if (n == cmdNum) break;
            if (n < 0) { cmdNum = n - 100; break; }
            if (getTime() >= doneTime) { cmdNum = -100; break; }
        }
    }
